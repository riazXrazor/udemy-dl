function download(dataObj,callback) {
  ...
  var vtt2srt = require('node-vtt-to-srt');
    ...
    var fileUrl = dataObj.data_url,
      ...
      apiPath = base_path + path.sep + sanitize(dataObj.chapter) + path.sep+filename +path.sep+ title,
      total_size = 0,
      total_downloaded = 0,
      remaining_time = 0,
      save_path = base_path+path.sep+sanitize(dataObj.chapter)+path.sep+filename
      ;
      console.log();
