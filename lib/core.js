...
var core_functions = require('./functions');
 ...
 let login = function(username, password,callback){
    ...
    let token;
    let login_url = `https://${argv.host}/api-2.0/auth/udemy-auth/login/?fields[user]=access_token`;
    _.forEach(session.headers, function(value, key) {
      ...
      let payload = {'email' : username, 'password': password}
        ...
        .then((httpResponse)=>{
            ...
            let josnRes = httpResponse.body;

           if(josnRes.access_token && josnRes.id)
           {
              session.set_auth_headers(argv.host, josnRes.access_token,josnRes.id)
              callback(josnRes.access_token,josnRes.id);
           }
           else {
...
let check_course_status = function(course_id,cb){
    ...
    let check_course_status_url = `https://${argv.host}/api-2.0/users/me/course-previews/?course=${course_id}`
        var options = {
  ...
  let set_video_resolution = function (element,callback) {
	...
	return;
      ...
      var get_url = `https://${argv.host}/api-2.0/users/me/subscribed-courses/${course_id}/lectures/${lecture_id}?fields[asset]=@min,download_urls,external_url,stream_urls,slide_urls&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4`;
  ...
  let extract_lecture_url = function(acc,element,index,callback){
      ...
      var get_url = `https://${argv.host}/api-2.0/users/me/subscribed-courses/${course_id}/lectures/${lecture_id}?fields[asset]=@min,download_urls,external_url,slide_urls&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4`;
      var options = {
        url: get_url,
        headers : session.headers
      };
  ...
  let get_data_links = function(course_id, lecture_start, lecture_end){
    ...
    try{
      ...
      /*set_video_resolution(setvidres,function () {
        ...
        set_video_resolution(setvidres,function () {
            var initializing = new Spinner('Initializing, please wait...          ');
                initializing.start();
                var tmp = 'udl-tmp/';
    ...
    if(item.type=='v')
    {
        ...
        if(item.attachments && item.attachments.length)
        {
            async.eachSeries(item.attachments,function(attachment,cb){
                ...
                var get_url = `https://${argv.host}/api-2.0/users/me/subscribed-courses/${course_id}/lectures/${lecture_id}/supplementary-assets/${attachment.id}?fields[asset]=@min,download_urls,stream_urls,external_url,slide_urls&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4`;
        ...
        var get_url = `https://${argv.host}/api-2.0/users/me/subscribed-courses/${course_id}/lectures/${lecture_id}?fields[asset]=@min,download_urls,stream_urls,external_url,slide_urls,captions,tracks&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4`;
        var options = {
        url: get_url,
        headers : session.headers
        };
 ...
 function get_course_list(callback){
    ...
    var get_url = `https://${argv.host}/api-2.0/users/me/subscribed-courses?page_size=50`;
