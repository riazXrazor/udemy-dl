          ...
          if(access_token && client_id)
           {
              ...
              callback(access_token,client_id);
           }
           else {
            save_debug_data(res, 'Cannot logging in', 'log');
             console.log("Cannot logging in");
             process.exit(0);
           }

    })
    .catch(r=>{
        save_debug_data(r, 'Error occured in login', 'log');
        console.log(r)
        let err = JSON.parse(r.response.text)
...
/*
    _.forEach(session.headers, function(value, key) {
        ...
        .then((httpResponse)=>{
           ...
           {
              session.set_auth_headers(host, josnRes.access_token,josnRes.id)
              callback(josnRes.access_token,josnRes.id);
           }
           else {
            save_debug_data(httpResponse, 'Cannot logging in', 'log');
             console.log("Cannot logging in");
             process.exit(0);
           }
...
let check_course_status = function(course_id,cb){
    ...
    let check_course_status_url = `${host}/api-2.0/users/me/course-previews/?course=${course_id}`
        var options = {
  ...
  let set_video_resolution = function (resolution_choices,callback) {
      ...
      var get_url = `${host}/api-2.0/users/me/subscribed-courses/${course_id}/lectures/${lecture_id}?fields[asset]=@min,download_urls,external_url,stream_urls,slide_urls&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4`;
      ...
      try{
          request.get(options,function(e,r,b){
            ...
            //   console.log(videos);
              save_debug_data(resolution_choices, 'udemy-dl-error', 'log');
              var questions = [
                  {
                      type: 'list',
                      name: 'resolution',
  ...
  let extract_lecture_url = function(acc,element,index,callback){
      ...
      var get_url = `${host}/api-2.0/users/me/subscribed-courses/${course_id}/lectures/${lecture_id}?fields[asset]=@min,download_urls,external_url,slide_urls&fields[course]=id,is_paid,url&fields[lecture]=@default,view_html,course&page_config=ct_v4`;
      var options = {
        url: get_url,
        headers : session.headers
      };
  ...
  let get_data_links = function(course_id, lecture_start, lecture_end){
    ...
    try{
      ...
      for (var i = 0; i < course_data_len; i++) {
        ...
        // console.log(resolution_choices)
        // process.exit(0);
            ...
            var initializing = new Spinner('Initializing, please wait...          ');
                ...
                try{
                    ...
                    async.eachSeries(download_queue,download,function(err){
                        if(err) {  
                            save_debug_data(err, 'udemy-dl-error', 'log'); console.log(chalk.red("Error downloading, Try again !!")); 
                        }
                ...
                catch(e)
                {
                    ...
                    async.eachSeries(all_videos_list,ready_for_download,function(err,result){
                        if(argv.export){
                            ...
                            initializing = new Spinner('Exporting, please wait...          ');
                            initializing.start();
                            jsonfile.writeFileSync(course_id+'.json',download_queue);
                            initializing.stop();
                            console.log(chalk.green("Done Exporting !!"))
                            process.exit(0)
                        }
 ...
 function ready_for_download(item,callback)
 {
    ...
    try{
        ...
        var url = list_videos[argv.resolution] ||  list_videos['Auto'] || list_videos['720'] || list_videos['360'];
        if(!url)
        {
 ...
 function get_course_list(callback){
    ...
    var get_url = `${host}/api-2.0/users/me/subscribed-courses?page_size=500`;
 ...
 function fetch_courses(url,cb)
 {
    ...
    request.get(options,function(e,r,b){
        if(e){
            console.log(e.toString())
            console.log(chalk.red("Error occured during fetching courses"));
            process.exit(0);
        }
