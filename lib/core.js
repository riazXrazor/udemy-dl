  ...
  let set_video_resolution = function (element,callback) {
      ...
      try{
          request.get(options,function(e,r,b){
              ...
              for(var i=0;i<json_source.asset.download_urls.Video.length;i++)
              {
                  list_videos[json_source.asset.download_urls.Video[i].label] = json_source.asset.download_urls.Video[i].file;
                  resolution_choices.push(json_source.asset.download_urls.Video[i].label);
              }
